#    winamp.py, expanded slightly by Avi Press, to add functionality for
#    winamp_suggest
#
#
...
class Winamp(object):
	...
	def playArtist(self, artist):
		"""Plays a given artist name. Written by Avi Press"""
		self.__sendCommandMessage(self.IPC_DELETE, 0, self.__playlistHWND)
		self.playlist = self.query("artist = \"%s\"" % artist)
		self.stop()
		self.sortPlaylist()
		length = self.getListLength()
		self.setPlaylistPosition(random.randint(0,length))
		self.play()
